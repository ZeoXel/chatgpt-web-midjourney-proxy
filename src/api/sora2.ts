    ...
    for(let i=0; i<200; i++){
        try{
            ...
            if (task.status === 'failed') {
                console.log('[Sora2] ❌ 任务失败:', JSON.stringify(task, null, 2));
            }
            mlog("sora2 task", a)
            ...
            if(task.status === 'completed' && task.url) {
                console.log('[Sora2 Video Save] ✅ 开始保存到COS:', task.url);
                ...
                saveVideoToCOS({
                    ...
                    console.log('[Sora2 Video Save] ✅ 保存成功');
                }).catch(err => {
                    console.error('[Sora2 Video Save] ❌ 保存失败:', err.message || err);
                });
            }
